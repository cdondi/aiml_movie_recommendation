...
def load_faiss_index():
    ...
    if _faiss_index is None:
        ...
        _faiss_index = faiss.read_index(
            faiss_db_path, faiss.IO_FLAG_MMAP
        )  # Memory-mapped FAISS
    return _faiss_index
...
def load_csv_from_s3(filename):
    ...
    return pd.read_csv(
        io.BytesIO(response["Body"].read()), low_memory=False
    )  # Efficient reading
...
def get_movie_vectors():
    ...
    if _movie_vectors is None:
        ...
        st.write(
            "___________ Convert sparse TFIDF Matrix to dense numpy array for FAISS  .........."
        )
...
def recommend_content_based(title, num_recommendations=10, min_similarity=0.4):
    ...
    distances, recommended_indices = faiss_index.search(
        query_vector, num_recommendations + 1
    )  # +1 to skip the input movie
...
def recommend_collaborative(user_id, num_recommendations=5):
    ...
    if user_id < 1 or user_id > user_factors.shape[0] or user_id is None:
        raise ValueError(
            f"Invalid user_id: {user_id}. Must be between 1 and {user_factors.shape[0]}."
        )
    ...
    movies = get_movies()
    recommended_titles = movies.loc[
        movies.index.isin(recommended_movie_indices), "title"
    ].tolist()
    return recommended_titles
...
def recommend_hybrid(title, user_id=None, content_weight=0.5, collab_weight=0.5, num_recommendations=10):
    ...
    if user_id is None:
        ...
        content_recommendations = recommend_content_based(
            title, num_recommendations=100, min_similarity=0.3
        )

        collaborative_recommendations = []

        combined_recommendations = (
            (pd.Series(content_recommendations).value_counts(normalize=True))
        ...
        content_recommendations = recommend_content_based(
            title, num_recommendations=100, min_similarity=0.4
        )
        collaborative_recommendations = recommend_collaborative(
            user_id, num_recommendations=100
        )
        ...
        combined_recommendations = (
            ...
            * pd.Series(content_recommendations).value_counts(normalize=True)
            + collab_weight
            * pd.Series(collaborative_recommendations).value_counts(normalize=True)
        ).sort_values(ascending=False)

    top_recommendations = combined_recommendations.head(
        num_recommendations
    ).index.tolist()
